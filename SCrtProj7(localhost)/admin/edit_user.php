...
<html>
    ...
    <body>
        ...
        Password: <input type = "text" name = "pswd" value = ' . $row['password'] . '><br>
        <input type = "submit" value = "Submit">
        </form>';
        }

        if (isset($_POST['name']) && $_POST['name'] !== $username) {
            $sql = "UPDATE client_entity SET username= '" . $_POST['name'] . " 'WHERE username = '" . $username . "'";
        ...
        if (isset($_POST['emailadd']) && $_POST['emailadd'] !== $row['email']) {
            $sql = "UPDATE client_entity SET email= '" . $_POST['emailadd'] . " 'WHERE username = '" . $username . "'";
            if ($conn->query($sql) === TRUE) {
        ...
        if (isset($_POST['pswd']) && $_POST['pswd'] !== $row['password']) {
            $sql = "UPDATE client_entity SET password= '" . $_POST['pswd'] . " 'WHERE username = '" . $username . "'";
            if ($conn->query($sql) === TRUE) {
